...
export default class SearchBarAnimation {
  ...
  topPartHeight = 350; // arrowHeight + inputHeight + padding (Top part)
  fullHeight = 450; // = 222
  ...
  maxActionAnimated = 0; // Location input height + padding (Bottom part)
  ...
  _createClampedScroll() {
   this.clampedScroll = this.scrollY.interpolate({ // Only positive
        inputRange: [100, this.topPartHeight +100],
        outputRange: [100, this.topPartHeight +100],
        extrapolate: 'clamp',
      });
  ...
  getOpacitySearchBar() {
    return {
      opacity: this.clampedScroll.interpolate({
        inputRange: [this.topPartHeight, this.maxClamp-200],
        outputRange: [1, 0],
        extrapolate: 'clamp',
      })
    };
    ...
    return {
      ...
      transform: [{
        translateY: Animated.add(
          this.actionAnimated.interpolate({
            inputRange: [0, this.maxActionAnimated],
            outputRange: [0, ifIphoneX(10, 0)],
            extrapolate: 'clamp',
          }),
          scroll.interpolate({
            inputRange: [0, 0],
            outputRange: [0, this.wrapperHeight + ifIphoneX(11, 0)],
            extrapolate: 'clamp'
          })
        )
      }]
